...
XML.prototype.addNamespace = function(ns) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (att.name().prefix == ns.prefix) {
      att._name = XML.getQName(org.apache.royale.utils.Language.string(att._name.child('localName')), null, org.apache.royale.utils.Language.string(att._name.child('uri')), true);
    }
  }
  return this;
};
...
XML.prototype.XML_appendChildInternal = function(child) {
  if (org.apache.royale.utils.Language.is(child, XMLList)) {
    ...
    for (var /** @type {number} */ i = 0; i < len; i++) {
      this.XML_appendChildInternal(child[0]);
    }
  } else {
    org.apache.royale.debugging.assertType(child, XML, "Type must be XML");
    child.setParent(this);
...
XML.prototype.attribute = function(attributeName) {
  ...
  attributeName = this.XML_toAttributeName(attributeName);
...
XML.prototype.setAttribute = function(attr, value) {
  ...
  if (attr.indexOf("xmlns") == 0) {
    ...
    var /** @type {QName} */ nameRef = this.XML_toAttributeName(attr);
...
XML.prototype.setChild = function(elementName, elements) {
  ...
  if (childType != "object") {
    var /** @type {XML} */ stringable = this.XML_xmlFromStringable(elements);
    chldrn = this.child(elementName);
    childIdx = (this.children().length() - 1) >> 0;
    if (chldrn.length())
      childIdx = (chldrn[0].childIndex() - 1) >> 0; else {
...
XML.prototype.setName = function(name) {
  ...
  this._name = XML.getQName(nameRef.localName, nameRef.prefix, nameRef.uri, ref == XML.ATTRIBUTE);
};
 ...
 * @return {QName}
 */
XML.prototype.XML_toAttributeName = function(name) {
...
var typeName = typeof(name);
  ...
  if (str.indexOf("@") == 0)
    return this.XML_toAttributeName(name);
  ...
  return {
    ...
    methods: function () {
      return {
        ...
        'removeChildAt': { type: 'void', declaredBy: 'XML', parameters: function () { return [ 'int', false ]; }},
        ...
        'setParent': { type: 'void', declaredBy: 'XML', parameters: function () { return [ 'XML', false ]; }},
